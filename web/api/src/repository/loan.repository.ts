...
export class LoanRepository implements ILoanRepository {
  ...
  async getSingleLoan(loanId: string, userId: string): Promise<ILoan | null> {
    return await Loan.findOne({ _id: loanId, userId: userId });
  }

  async deleteLoan(loanId: string, userId: string): Promise<ILoan | null> {
    return await Loan.findOneAndDelete({ _id: loanId, userId: userId });
  }
  ...
  async updateLoan(
    ...
    return await Loan.findOneAndUpdate({ _id: loanId, userId: userId }, body, {
      runValidators: true,
